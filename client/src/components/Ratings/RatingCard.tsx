...
const RatingCard: React.FC<RatingCardProps> = ({ review }) => {
    ...
    const fetchUserData = async () => {       
        try {
            const fetchedReviewUserData: UserData | null = await getUser(review.user_sub_id);

            if (!fetchedReviewUserData) {
                throw new Error("The user for requested review does not exist!");
            }
        ...
        <div className="flex flex-row gap-4 py-4 border-b-2">
            <div className="flex flex-col gap-3 w-5/12 min-w-36">
                <div className="flex flex-row gap-3">
                    <img src={reviewUserData?.profile_picture_url} className="size-10 rounded-xl object-cover" />
            ...
            <div className="flex flex-col w-full">
                <div className="flex flex-row gap-2 pb-2 items-center">
                    <ReviewStarList rating={review.rating} />
